      ...
      <div className="px-6 pt-12 pb-8">
        <div className="max-w-lg mx-auto stagger-in">
          ...
          {archetype && (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-[#8a7a66] mb-0.5">Your archetype</p>
                  <p className="font-heading text-lg font-semibold text-[#1a1008]">{archetype.name}</p>
                  <p className="text-xs text-[#8a7a66] italic">{archetype.tagline}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-4 shadow-sm text-center">
              <p className="text-2xl font-semibold text-[#1a1008]">{stats?.streakDays ?? 0}</p>
              <p className="text-[10px] uppercase tracking-wider text-[#8a7a66] mt-1">Day streak</p>
            </div>
            <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-4 shadow-sm text-center">
              <p className="text-2xl font-semibold text-[#1a1008]">{stats?.totalMessages ?? 0}</p>
              <p className="text-[10px] uppercase tracking-wider text-[#8a7a66] mt-1">Messages</p>
            </div>
            <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-4 shadow-sm text-center">
              <p className="text-2xl font-semibold text-[#1a1008]">{stats?.totalJournalEntries ?? 0}</p>
              <p className="text-[10px] uppercase tracking-wider text-[#8a7a66] mt-1">Journal</p>
            </div>
          </div>

          {/* ── Weekly Activity Chart ── */}
          <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-5 shadow-sm mb-6">
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: isCurrentWeek
                          ? "linear-gradient(180deg, #4a7c6b, #2d4e43)"
                          : "#d4e6df",
                      }}
                    />
                    <span className="text-[9px] text-[#c4bbaf]">
            ...
            <p className="text-xs text-[#c4bbaf] mt-3">Messages sent per week (last 7 weeks)</p>
          </div>
                    ...
                    return (
                      <div key={mood} className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="h-3 bg-[#f0ece4] rounded-full overflow-hidden">
                            <div
                              className="h-full bg-gradient-to-r from-[#4a7c6b] to-[#6b9c8a] rounded-full transition-all"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
                        <span className="text-xs text-[#8a7a66] w-10 text-right">{pct}%</span>
                      </div>
                    );
                  })}
              </div>
            ...
            <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-5 shadow-sm mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {stats.topTags.map(({ tag, count }) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1.5 text-sm font-medium text-[#2d4e43] bg-[#d4e6df] px-3 py-1.5 rounded-full"
                  >
                    {tag}
                    <span className="text-[10px] text-[#4a7c6b]/60">{count}</span>
                  </span>
                ))}
              </div>
            </div>
          ...
          {archetype && (
            <div className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-5 shadow-sm mb-6">
              <p className="text-xs font-medium tracking-wide uppercase text-[#c4849c] mb-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className="bg-white/70 backdrop-blur-sm border border-[#e8e4df] rounded-2xl p-4 shadow-sm text-left hover:bg-white hover:shadow-md transition-all"
            >
              <span className="text-xl mb-2 block">📓</span>
              <p className="text-sm font-semibold text-[#1a1008]">Journal</p>
              <p className="text-xs text-[#8a7a66]">Log a moment</p>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <p className="text-sm font-semibold text-[#1a1008]">Exercises</p>
              <p className="text-xs text-[#8a7a66]">Practice a skill</p>
            </button>
          </div>
        </div>
      </div>
